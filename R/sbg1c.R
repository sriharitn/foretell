...
## -------- helpers --------
#' @keywords internal
.sbg_sigmoid <- function(x) 1 / (1 + exp(-x))
.sbg_logit   <- function(p) log(p / (1 - p))
.sbg_clip01  <- function(p, eps = 1e-8) pmin(1 - eps, pmax(eps, p))

# survival probabilities S(0..n_last) for sgm with a,b, + one-and-done & cure
#' @keywords internal
.sbg_surv_prob_from_pars <- function(a, b, d = 0, cure = 0, n_last) {
  stopifnot(is.finite(a), is.finite(b), a > 0, b > 0)
  a  <- max(a, 1e-12)
  b  <- max(b, 1e-12)
  k <- 0:n_last
  S_base <- exp(lbeta(a, b + k) - lbeta(a, b))
  if (n_last >= 1) {
...
#' @keywords internal
.sbg_convert_scale <- function(S, object, scale = c("input","percent","count","prob")) {
...
.sbg_label_for_scale <- function(scale, object) {
...
#' @keywords internal
.sbg_make_pd <- function(S, eps_floor = 1e-8) {
  S <- as.matrix((S + t(S)) / 2)
...
#' @keywords internal
.sbg_J_params <- function(theta, est_d, est_cure) {
  ...
  f <- function(th) {
    ...
    d    <- if (est_d)    { v <- .sbg_clip01(.sbg_sigmoid(th[i])); i <- i + 1L; v } else 0
    cure <- if (est_cure) .sbg_clip01(.sbg_sigmoid(th[i])) else 0
    ab_sum <- (1/q) - 1
    a <- m * ab_sum
    b <- (1 - m) * ab_sum
    c(a, b, if (est_d) d, if (est_cure) cure)
  }
  numDeriv::jacobian(f, theta)
}
.sbg_J_repar <- function(theta, est_d, est_cure) {
  if (!requireNamespace("numDeriv", quietly = TRUE)) stop("Package 'numDeriv' is required for Jacobians.")
  f <- function(th) {
    i <- 1L
    m    <- .sbg_clip01(.sbg_sigmoid(th[i])); i <- i + 1L
    ...
    d    <- if (est_d)    { v <- .sbg_clip01(.sbg_sigmoid(th[i])); i <- i + 1L; v } else 0
    cure <- if (est_cure) .sbg_clip01(.sbg_sigmoid(th[i])) else 0
    c(m = m, q = q, if (est_d) d = d, if (est_cure) cure = cure)
  }
  numDeriv::jacobian(f, theta)
}
...
# -----------------------------------------------------------------------
.sbg_optimize <- function(theta0, nll) {
  # 1) BFGS
  out <- try(stats::optim(theta0, fn = nll,
                          control = list(reltol = 1e-10)),
             silent = TRUE)
  if (!inherits(out, "try-error")) return(out)

  # 2) L-BFGS-B
  out <- try(stats::optim(theta0, fn = nll, method = "L-BFGS-B"),
             silent = TRUE)
  if (!inherits(out, "try-error")) return(out)
  ...
  # harmonize to optim-like
  list(par = res$par %||% res$solution,
       value = res$value,
...
#'
#' @return An object of class `sbg_fit`.
#'
...
sbg1c <- function(
  ...
  theta0 <- c(
    .sbg_logit(starts_m),
    .sbg_logit(starts_q),
    if (est_d)    .sbg_logit(starts_d)    else NULL,
    if (est_cure) .sbg_logit(starts_cure) else NULL
  )
  ...
  nll <- function(theta) {
    ...
    m    <- .sbg_sigmoid(th_m);    q <- .sbg_sigmoid(th_q)
    d    <- if (est_d)    .sbg_sigmoid(th_d)   else 0
    cure <- if (est_cure) .sbg_sigmoid(th_cur) else 0

    ab_sum <- (1/q) - 1
    a <- max(m * ab_sum, 1e-12)
    b <- max((1 - m) * ab_sum, 1e-12)

    S  <- .sbg_surv_prob_from_pars(a, b, d, cure, n_last = Tobs - 1)
    p  <- pmax(-diff(S), 1e-12)
    ST <- max(S[Tobs], 1e-12)
  ...
  # optimize (with harmonized fallbacks)
  opt <- .sbg_optimize(theta0, nll)
  ...
  m_hat    <- .sbg_sigmoid(th_m)
  q_hat    <- .sbg_sigmoid(th_q)
  d_hat    <- if (est_d)    .sbg_sigmoid(th_d)   else 0
  cure_hat <- if (est_cure) .sbg_sigmoid(th_cur) else 0
  ...
  # fitted & projected
  S_fit <- .sbg_surv_prob_from_pars(a_hat, b_hat, d_hat, cure_hat, n_last = Tobs - 1)
  fitted <- if (input == "percent") S_fit * 100 else S_fit * N0

  if (h > 0) {
    S_all  <- .sbg_surv_prob_from_pars(a_hat, b_hat, d_hat, cure_hat, n_last = Tobs - 1 + h)
    S_proj <- S_all[(Tobs + 1):(Tobs + h)]
    projected <- if (input == "percent") S_proj * 100 else S_proj * N0
  } else projected <- numeric(0)

  # SEs via Hessian in theta-space (robust inversion); check only d/cure boundary
  ...
  if (do_se) {
    ...
    if (!inherits(H, "try-error")) {
      ...
      if (!requireNamespace("numDeriv", quietly = TRUE)) {
        ...
        theta_hat <- c(th_m, th_q, if (est_d) th_d, if (est_cure) th_cur)
        Jp <- .sbg_J_params(theta_hat, est_d, est_cure)
        Jr <- .sbg_J_repar (theta_hat, est_d, est_cure)
        ...
        names(se_params) <- c("a","b", if (est_d) "d", if (est_cure) "cure")
        names(se_repar)  <- c("m","q", if (est_d) "d", if (est_cure) "cure")
      }
    }
  }
  ...
  obj <- list(
    ...
    coef_params = setNames(c(a_hat, b_hat, if (est_d) d_hat, if (est_cure) cure_hat),
                           c("a","b", if (est_d) "d", if (est_cure) "cure")),
    coef_repar  = setNames(c(m_hat, q_hat, if (est_d) d_hat, if (est_cure) cure_hat),
                           c("m","q", if (est_d) "d", if (est_cure) "cure")),
    logits      = setNames(c(th_m, th_q, if (est_d) th_d, if (est_cure) th_cur),
                           c("theta_m","theta_q", if (est_d) "theta_d", if (est_cure) "theta_cure")),
    fitted      = fitted,
    projected   = projected,
    logLik      = -opt$value,
...
#' @export
#' @method print sbg_fit
print.sbg_fit <- function(x, digits = 4, ...) {
  cat("Shifted Beta-Geometric (sBG)\n")
...
#' @export
#' @method summary sbg_fit
summary.sbg_fit <- function(object, digits = 4, ...) {
  ...
  class(out) <- "summary.sbg_fit"
  out
}

#' @export
#' @method print summary.sbg_fit
print.summary.sbg_fit <- function(x, digits = 4, ...) {
  cat("Shifted Beta-Geometric (sBG) summary\n")
  cat("LogLik:", round(x$logLik, digits), "  Convergence:", x$convergence, "\n")
  if (!is.null(x$note)) cat("Note:", x$note, "\n")
  cat("\nOriginal parameters (a, b[, d][, cure]):\n")
  if (!is.null(x$params)) print(round(x$params, digits)) else cat("(not available)\n")
  cat("\nReparameterization (m, q[, d][, cure]):\n")
...
#' @export
#' @method coef sbg_fit
coef.sbg_fit <- function(object, reparam = FALSE, ...) {
  if (isTRUE(reparam)) object$coef_repar else object$coef_params
}

#' @export
#' @method vcov sbg_fit
vcov.sbg_fit <- function(object, what = c("params","repar","theta"), ...) {
  what <- match.arg(what)
...
#' @export
#' @method logLik sbg_fit
logLik.sbg_fit <- function(object, ...) {
  val <- object$logLik
...
#' @export
#' @method fitted sbg_fit
fitted.sbg_fit <- function(object, scale = c("input","percent","count","prob"), ...) {
  scale <- match.arg(scale)
  ...
  S <- .sbg_surv_prob_from_pars(a, b, d, cure, n_last = Tobs - 1)
  .sbg_convert_scale(S[1:Tobs], object, scale)
}

#' @export
#' @method predict sbg_fit
predict.sbg_fit <- function(object, n_ahead = NULL, scale = c("input","percent","count","prob"), ...) {
  scale <- match.arg(scale)
  Tobs <- length(object$y)
  ...
  d    <- if ("d"    %in% names(p)) p["d"]    else 0
  cure <- if ("cure" %in% names(p)) p["cure"] else 0
  S <- .sbg_surv_prob_from_pars(a, b, d, cure, n_last = Tobs - 1 + h)
  .sbg_convert_scale(S[(Tobs + 1):(Tobs + h)], object, scale)  # next periods
}

#' @export
#' @method plot sbg_fit
plot.sbg_fit <- function(x,
                         ...
                         lwd_band_border = 1.2,    # boundary line width for polygons
                         draw_band_center = TRUE,  # draw S_hat center line if present
                         lty_band_center  = 3,     # center line style
                         lwd_band_center  = 1.5,   # center line width
  ...
  # fitted/proj
  S_fit <- .sbg_surv_prob_from_pars(a, b, d, cure, n_last = Tobs - 1)
  y_fit <- .sbg_convert_scale(S_fit[1:Tobs], x, scale)

  y_proj <- numeric(0); t_prj <- integer(0)
  if (!is.null(h) && h > 0) {
    S_all  <- .sbg_surv_prob_from_pars(a, b, d, cure, n_last = Tobs - 1 + h)
    S_proj <- S_all[(Tobs + 1):(Tobs + h)]
    y_proj <- .sbg_convert_scale(S_proj, x, scale)
    t_prj  <- Tobs:(Tobs + h - 1)
  }

  t_obs <- 0:(Tobs - 1)

  if (is.null(ylab)) ylab <- .sbg_label_for_scale(scale, x)
  if (is.null(main)) {
    od <- if (isTRUE(x$flags["one_and_done"])) " + one-and-done" else ""
    cu <- if (isTRUE(x$flags["cure"]))        " + cure"         else ""
    main <- paste0("Shifted Beta-Geometric", od, cu)
  }
  ...
  # helper to draw one polygon band with borders + optional center line
  draw_poly_band <- function(df, col_fill, col_border, alpha) {
    ...
    # optional center (S_hat) if present
  ...
  # draw polygons (order: BOTH at back, then EST, then FUND) so overlaps look reasonable
  ...
  # vertical boundary at last observed
